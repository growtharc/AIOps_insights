...
import { createChatSession, sendMessage, type ChatSession, appendHistoryNote, getHistorySummary, retrySQLWithError } from './services/openaiService';
import { requiresMultiStepAnalysis, performComprehensiveAnalysis } from './services/aiAnalysisService';
...
function App() {
  ...
  const [messages, setMessages] = useState<Message[]>([getInitialMessage(false)]);
  ...
  const handleRefresh = async () => {
    ...
    setChat(null);
    setMessages([{ id: 0, role: 'assistant', isLoading: true, content: 'Fetching data from Jira...' }]);
    ...
    try {
      ...
      const previewRes = await fetch('http://localhost:8080/query/execute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ session_id: newSessionId, sql: 'SELECT * FROM data LIMIT 5' }),
      });
      const preview = await previewRes.json();
      const chartData = (preview.rows as any[]).map((row: any[]) => {
        const obj: any = {};
        preview.columns.forEach((c: string, i: number) => { obj[c] = row[i]; });
        return obj;
      });
      ...
      setMessages([getInitialMessage(false), { id: 1, role: 'assistant', isError: true, error: `Failed to load data from Jira: ${error}` }]);
    } finally {
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    const loadingMessage: Message = { id: Date.now() + 1, role: 'assistant', isLoading: true };
        ...
        if (needsComprehensiveAnalysis) {
          ...
          const executeQuery = async (sql: string) => {
            ...
            return (body.rows as any[]).map((row: any[]) => {
              const obj: any = {};
              (body.columns as string[]).forEach((c: string, i: number) => { obj[c] = row[i]; });
              return obj;
            });
          };
          ...
          if (analysis.isSummaryOnly) {
            ...
            appendHistoryNote(chat, `Insight: Summary provided for: "${userMessage.content}"`);
          } else {
            ...
            const newAssistantMessage: Message = {
              ...
              content: {
                ...
                chartData,
                recommendation: analysis.recommendation,
                sqlQuery: analysis.steps.map(s => `-- Step ${s.step}: ${s.action}\n${s.sqlQuery}`).join('\n\n')
              },
            };
            setMessages(prev => prev.map(msg => msg.id === loadingMessage.id ? newAssistantMessage : msg));
            const priorSqls = analysis.steps.filter(s => s.sqlQuery).map(s => s.sqlQuery).join(' ; ');
          ...
          let assistantResponse = await sendMessage(chat, userMessage.content as string);
            ...
            try {
              ...
              if (!res.ok) {
                ...
                if (retryCount < MAX_RETRIES) {
                  console.log(`SQL failed (attempt ${retryCount + 1}), retrying with error feedback...`);
                  assistantResponse = await retrySQLWithError(chat, userMessage.content as string, errorMsg, sql);
              ...
              if (retryCount >= MAX_RETRIES || fetchError.message.includes('Failed to fetch')) {
                throw fetchError;
              }
              retryCount++;
            }
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 text-gray-900 font-sans">
      <header className="p-4 border-b border-gray-200 shadow-sm sticky top-0 bg-white backdrop-blur-md z-10">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
            <div className="flex items-center space-x-3 -ml-8">
              <img src="/logo.png" alt="Logo" className="h-8" />
              <h1 className="text-xl font-bold text-gray-800">AIOps Insights</h1>
            </div>
            <div className="flex items-center space-x-2">
                <button
                    ...
                    className={`font-semibold py-2 px-4 rounded-lg transition-colors text-sm shadow-sm
                        ${isRefreshing
      ...
      <footer className="p-4 border-t border-gray-200 sticky bottom-0 bg-white">
        <div className="max-w-4xl mx-auto">
          <form onSubmit={handleSendMessage} className="flex items-center space-x-2 bg-white border border-gray-300 rounded-lg p-2 focus-within:ring-2 focus-within:ring-blue-500 shadow-sm">
            <input
              type="text"
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder={isDataLoaded ? "Ask a question about your data..." : "Please refresh data to get started"}
              className="flex-1 bg-transparent focus:outline-none px-2 text-gray-900 placeholder:text-gray-400"
              disabled={isLoading || !isDataLoaded || !chat}
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim() || !isDataLoaded || !chat}
              className="p-2 rounded-md bg-blue-600 text-white disabled:bg-gray-300 disabled:cursor-not-allowed hover:bg-blue-700 transition-colors"
            >
